...
public class SignalCollection {
    ...
    public SignalBuffer get_max_signal(int s,int e) throws Exception {

        SignalBuffer buffer=new SignalBuffer(size);
        for (int j = 0; j < size; j++) {

            double m = Double.MIN_VALUE;
            for (int i = s; i < e; i++) {

                double v = this.get___idx(i).get___idx(j);
                if(v>m)
                    m=v;

            }
            buffer.set___idx(j,m);
        }
        return buffer;
    }
    public SignalBuffer get_energy_signal(int s,int e) throws Exception {
        ...
        for (int j = 0; j < size; j++) {
            ...
            buffer.set___idx(j,Math.sqrt(m));
        }
        return buffer;
    }
}
