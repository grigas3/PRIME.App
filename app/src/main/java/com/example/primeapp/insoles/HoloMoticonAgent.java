package com.example.primeapp.insoles;
...
public class HoloMoticonAgent   extends AppCompatActivity implements IDataCollector, IDataShink {
    ...
    //Definitions
    private int lastLeftPressure=0;
    private int lastRightPressure=0;
    private double sumCOP=0;
    private int samples=0;
    private int MaxSamples=4096;



    private static int MaxPressure(Common.DataMessage data)
    {

        List<Integer> pressures=data.getPressureList();
        return Collections.max(pressures);
    ...
    private BroadcastReceiver createServiceMsgReceiver() {
        ...
        BroadcastReceiver br = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (moticonMessage.getMsgCase().name() == "DATA_MESSAGE")
                {
                    Common.DataMessage data=moticonMessage.getDataMessage();
                    ...
                    if(data.getSide()== Common.Side.LEFT)
                    {



                        lastLeftPressure=MaxPressure(data);


                        try {
                            PressureData pdata=new PressureData(lastLeftPressure,lastRightPressure);
//                            Log.d("insole", data.toString());

                            Log.d("insole",String.format( "L t:%d x: %.3f, y: %.3f z: %.3f",data.getTime(),data.getAcceleration(0),data.getAcceleration(1),data.getAcceleration(2)));
                            /*signalCollection.get___idx("PRESSURE").setValue(0,samples,lastLeftPressure);
                            signalCollection.get___idx("PRESSURE").setValue(1,samples,lastRightPressure);
                            samples++;
                    ...
                    else
                    {
                        ...
                        Log.d("insole",String.format( "R t:%d x: %.3f, y: %.3f z: %.3f",data.getTime(),data.getAcceleration(0),data.getAcceleration(1),data.getAcceleration(2)));
                        lastRightPressure=MaxPressure(data);
                    }
...
//Init Data
        ...
        MSGSENDER messageSender = new MSGSENDER();
    ...
    NamedSignalCollection signalCollection=null;
    private void initData() throws Exception {


        if(signalCollection==null) {
            signalCollection = new NamedSignalCollection();
            signalCollection.set___idx("PRESSURE", new SignalCollection(2, MaxSamples));
